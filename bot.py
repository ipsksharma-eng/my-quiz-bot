...
def _generate_quiz_pdf(quiz, questions):
    ...
    # ── TEXT SHAPING: PEHLE enable karo, BAAD MEIN font add karo ──────────────
    # CRITICAL FIX: set_text_shaping MUST be called before add_font
    # Devanagari ke liye script="Deva" aur language="HIN " zaroori hai
    try:
        pdf.set_text_shaping(True)
        logging.info("PDF: HarfBuzz text shaping enabled (Devanagari mode)")
    except Exception as e:
        logging.error(f"PDF: Text shaping FAILED: {e}. Run: pip install uharfbuzz")
...
def _export_pdf_quizpdf(chat_id, quiz_id):
    ...
    try:
        ...
        # ── PDF bhejne ke baad status message delete karo ──────────────────
        if _pdf_status_msg:
...
def _countdown_and_start(chat_id, uid, quiz_id, show_countdown=True):
    ...
    send_next_poll(sid)
    # ── GO message quiz shuru hone ke baad delete karo ──────────────────────
    if _go_msg_id:
...
def handle_text(msg):
    ...
    if state == "awaiting_quiz_title":
        ...
        # ── Saving message delete karo quiz card aa jaane ke baad ─────────
        if _saving_msg:
