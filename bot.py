...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║          QuizBot Pro — v4.8  (/quizpdf ReportLab Professional PDF)           ║
...
║  ✅ Separator ─ ─ ─ between every student in leaderboard                    ║
║  ✅ /quizpdf → ReportLab PDF: dark-blue header, answer green box each Q     ║
║  ✅ /testseries → same PDF (alias)                                           ║
║  ✅ Hindi font: NotoSansDevanagari auto-loaded if .ttf present               ║
...
def _load_session_questions(sess):
    with get_db() as conn:
        all_qs = conn.execute("SELECT * FROM questions WHERE quiz_id=? ORDER BY position, question_id", (sess["quiz_id"],)).fetchall()
    order = json.loads(sess["shuffled_order"] or "[]")
    if order:
        qmap = {q["question_id"]: q for q in all_qs}
        return [qmap[qid] for qid in order if qid in qmap]
    return list(all_qs)
...
def _auto_advance(session_id, expected_q_idx):
    try:
        ...
        with get_db() as conn:
            sess = conn.execute("SELECT current_q_idx, is_paused, is_completed FROM active_sessions WHERE session_id=?", (session_id,)).fetchone()
        if not sess or sess["is_completed"] or sess["is_paused"]: return
        if sess["current_q_idx"] == expected_q_idx:
            t = threading.Thread(target=send_next_poll, args=(session_id,), daemon=True)
            t.start()
    except Exception as e:
...
def send_next_poll(session_id):
    ...
    try:
        with get_db() as conn:
            ...
            if not sess or sess["is_paused"] or sess["is_completed"]: return
            quiz = conn.execute("SELECT * FROM quizzes WHERE quiz_id=?", (sess["quiz_id"],)).fetchone()

        questions = _load_session_questions(sess)
        total     = len(questions)
        ...
        if ref:
            ...
            try: bot.send_message(sess["chat_id"], f"{ref_header}\n\n{ref}", parse_mode="Markdown")
            except Exception: bot.send_message(sess["chat_id"], f"Reference Q{q_idx+1}/{total}\n\n{ref}")
        ...
        try:
            with get_db() as conn:
                conn.execute("UPDATE active_sessions SET current_q_idx=? WHERE session_id=?", (q_idx+1, session_id))
            t = threading.Timer(2.0, _auto_advance, args=[session_id, q_idx + 1])
            t.daemon = True
            t.start()
            _auto_timers[session_id] = t
        except Exception: pass
...
def _send_leaderboard(session_id, chat_id, quiz_title, neg_val, total_q, session_start):
    ...
    # ── Podium helpers (from new bot layout) ─────────────────────────────────
    ...
    SEP  = "━━━━━━━━━━━━━━━━━━━━━━━━"
    SEP2 = "─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─"
    W    = 32   # monospace podium width

    # ── PODIUM: 1st center, then 2nd-3rd sides ───────────────────────────────
    podium = ""
    if len(players) >= 1:
        r1 = players[0]
    ...
    # ── LEADERBOARD: name / stats / timing per player, separator between all ─
    rank_icons = {1: "👑", 2: "🥈", 3: "🥉"}
    rank_lines = []
...
def group_only_reply(msg):
    if is_group(msg):
        bot.send_message(msg.chat.id, "ℹ️ Quiz creation aur settings ke liye mujhe *private chat* mein message karo!\nGroup mein quiz chalane ke liye sirf card par diye gaye link jaise `/startquiz_1` use karo.", parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
        return True
    return False
...
def cmd_start(msg):
    ...
    if is_group(msg):
        if len(parts) > 1 and parts[1].startswith("quiz_"):
            ...
            else: bot.send_message(msg.chat.id, "👋 Is group mein abhi tak koi quiz nahi hua.\nQuiz start karne ke liye creator se `/startquiz_ID` bhejne ko kaho.", parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
        return
    ...
    if OWNER_ID is None:
        print(f"[OWNER SETUP] Apna OWNER_ID set karo: OWNER_ID = {uid}")
...
def cmd_done(msg):
    ...
    if not store.get("questions"): return safe_send(msg.chat.id, "No questions buffered yet. Paste questions first.")
...
def cmd_testseries(msg):
    ...
    if len(parts) > 1 and parts[1].strip().isdigit():
        ...
        safe_send(msg.chat.id, "📄 *Quiz PDF*\n\nQuiz ID bhejo — PDF mein har question ke niche answer dikhenga.", parse_mode="Markdown")
...
def handle_callback(call):
    ...
    if data.startswith("qs_"):
        ...
        with get_db() as conn:
            ...
            if action == "q":
                new_val = 1 - quiz["shuffle_q"]
                conn.execute("UPDATE quizzes SET shuffle_q=? WHERE quiz_id=?", (new_val, quiz_id))
            elif action == "o":
                new_val = 1 - quiz["shuffle_o"]
                conn.execute("UPDATE quizzes SET shuffle_o=? WHERE quiz_id=?", (new_val, quiz_id))
            quiz = conn.execute("SELECT * FROM quizzes WHERE quiz_id=?", (quiz_id,)).fetchone()
        ...
        short = data[7:]
        bot.answer_callback_query(call.id, f"ID: {short} — copy kar lo!", show_alert=True)
...
def cmd_ban(msg):
    if not is_owner(msg.from_user.id):
        return bot.send_message(msg.chat.id, "🚫 Sirf owner kar sakta hai.")
    parts = msg.text.split(maxsplit=2)
    if len(parts) < 2 or not parts[1].lstrip("-").isdigit():
        return bot.send_message(msg.chat.id, "Usage: <code>/ban USER_ID reason</code>", parse_mode="HTML")
...
def cmd_unban(msg):
    ...
    if len(parts) < 2 or not parts[1].lstrip("-").isdigit():
        return bot.send_message(msg.chat.id, "Usage: <code>/unban USER_ID</code>", parse_mode="HTML")
    target_id = int(parts[1])
    with get_db() as conn:
    ...
    if deleted:
        ...
        bot.send_message(msg.chat.id, f"⚠️ <code>{target_id}</code> banned nahi tha.", parse_mode="HTML")

@bot.message_handler(commands=["users"])
def cmd_users(msg):
    if not is_owner(msg.from_user.id):
        return bot.send_message(msg.chat.id, "🚫 Sirf owner dekh sakta hai.")
    with get_db() as conn:
        total  = conn.execute("SELECT COUNT(*) FROM users").fetchone()[0]
        banned = conn.execute("SELECT COUNT(*) FROM banned_users").fetchone()[0]
        recent = conn.execute(
            "SELECT user_id, first_name, username, created_at FROM users ORDER BY created_at DESC LIMIT 10"
        ).fetchall()
...
def cmd_banlist(msg):
    ...
    with get_db() as conn:
        rows = conn.execute(
            "SELECT b.user_id, b.reason, b.banned_at, u.first_name, u.username "
            "FROM banned_users b LEFT JOIN users u ON b.user_id=u.user_id ORDER BY b.banned_at DESC"
        ).fetchall()
    if not rows:
        return bot.send_message(msg.chat.id, "✅ Koi banned user nahi hai.")
...
def handle_text(msg):
    ...
    if state == "awaiting_quiz_title":
        ...
        clean = text.strip().lower()
        if clean not in ("free","paid"): return safe_send(msg.chat.id, "Send `free` or `paid`.", parse_mode="Markdown")
        store = _wizard.get(uid, {})
        qs, title, neg, timer = store.get("questions", []), store.get("title", "Untitled Quiz"), store.get("neg", "0"), store.get("timer", 45)
        if not qs:
            set_state(uid,"adding_questions")
        ...
        set_state(uid, "idle")
        qid = int(text)
        threading.Thread(target=_export_pdf_testseries, args=(msg.chat.id, qid), daemon=True).start()
...
def handle_poll_answer(poll_answer):
    ...
    with get_db() as conn:
        ...
        already    = conn.execute(
            "SELECT result_id FROM session_results WHERE session_id=? AND user_id=? AND question_id=?",
            (pm["session_id"], uid, pm["question_id"])).fetchone()

        if not already:
            conn.execute(
                "INSERT INTO session_results(session_id,user_id,participant_name,question_id,selected_idx,is_correct) "
                "VALUES(?,?,?,?,?,?)",
                (pm["session_id"], uid, name, pm["question_id"], selected, is_correct))
...
def handle_inline_query(inline_query):
    ...
    with get_db() as conn:
        quizzes = conn.execute(
            "SELECT z.*, COUNT(q.question_id) AS q_count FROM quizzes z "
            "LEFT JOIN questions q ON z.quiz_id=q.quiz_id "
            "WHERE z.creator_id=? GROUP BY z.quiz_id ORDER BY z.created_at DESC LIMIT 50",
            (uid,)
        ).fetchall()
    ...
    for quiz in quizzes:
        ...
        if q_text and q_text not in title and q_text not in short_id:
            continue
        ...
        card_text = (f"💳 <b>Quiz Name:</b> {safe_title}\n"
                     f"#️⃣ <b>Questions:</b> {quiz['q_count']}\n"
                     f"⏰ <b>Timer:</b> {quiz['timer_seconds']}s\n"
                     f"🆔 <b>Quiz ID:</b> <code>{quiz['short_id']}</code>\n"
                     f"🏴‍☠️ <b>-ve:</b> {neg_display}\n"
                     f"💰 <b>Type:</b> {quiz['quiz_type']}")
        ...
        res = InlineQueryResultArticle(
            id=str(quiz["quiz_id"]),
            title=quiz["title"],
            description=f"{quiz['q_count']} Qs | {quiz['timer_seconds']}s timer | ID: {quiz['short_id']}",
...
# ══════════════════════════════════════════════════════════════════════════════
#  MAIN EXECUTION (With Auto-Restart & Keep-Alive)
# ══════════════════════════════════════════════════════════════════════════════
