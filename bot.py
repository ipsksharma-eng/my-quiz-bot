...
def cmd_testseries(msg):
    ...
    register_user(msg)
    uid, parts = msg.from_user.id, msg.text.split(maxsplit=1)
    if len(parts) > 1 and parts[1].strip().isdigit(): _export_txt(msg.chat.id, int(parts[1].strip()))
    else:
        set_state(uid, "awaiting_txt_id")
        safe_send(msg.chat.id, "Send the *Quiz ID* to export as TXT.", parse_mode="Markdown")
...
def handle_text(msg):
    ...
    if state == "awaiting_quiz_title":
        ...
        if not text.isdigit(): return safe_send(msg.chat.id, "Enter a valid numeric Quiz ID.")
        set_state(uid, "idle"); _export_txt(msg.chat.id, int(text))
