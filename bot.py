...
#
#  POLLING ka problem:
#    - Render pe bot sone ke baad reconnect karta hai → messages miss hote hain
#    - "409 Conflict" error: do polling loops ek saath → bot hang
#    - Cronjob ping karne ke baad bhi 2-5 sec missed window rehta hai
#
#  WEBHOOK ka fayda:
#    - Telegram khud message push karta hai Flask pe → koi miss nahi
#    - Render ka PORT directly use hota hai — keep-alive separately nahi chahiye
#    - 409 Conflict ka koi issue hi nahi
#
#  SETUP (ek baar karna hai):
#    Render Environment Variables mein ye add karo:
#      BOT_TOKEN     = aapka bot token
#      DATABASE_URL  = Supabase PostgreSQL URL
#      RENDER_URL    = https://aapki-app-ka-naam.onrender.com
#                      (Render dashboard mein milega — trailing slash mat daalo)
...
# ── v5.3 FIX: Bot restart ke baad incomplete sessions clean karo ─────────────
# Warna purani sessions forever "is_completed=0" rehti hain aur /startquiz
# "Active session exists. Use /stop first." error deta hai — even fresh restart ke baad.
...
def create_quiz_and_save(uid, title, questions, neg="0", quiz_type="free", timer=45):
    # v5.3 FIX: Short ID uniqueness — O(1) DB check instead of fetching all IDs
    with get_db() as conn:
...
def send_next_poll(session_id):
    ...
    # v5.3 FIX: Initialize before try block — warna except mein NameError aata hai
    sess, q_idx = None, 0
...
def _generate_quiz_pdf(quiz, questions):
    ...
    if os.path.exists(font_reg):
        ...
        logging.info("PDF: NotoSansDevanagari font loaded")
    else:
    ...
    try:
        ...
        logging.info("PDF: HarfBuzz text shaping ON — script=Deva, lang=hi")
    except Exception as e:
        logging.error(f"PDF: Text shaping FAILED: {e}. Run: pip install uharfbuzz>=0.39.0")
    ...
    for q in questions:
        ...
        if ref:
            ...
            pdf.multi_cell(176, 5, ref_fmt, fill=True, new_x="LMARGIN", new_y="NEXT")
            ref_end = pdf.get_y()
...
def _export_pdf_quizpdf(chat_id, quiz_id):
    ...
    try: _pdf_status_msg = bot.send_message(chat_id, "⏳ <b>PDF ban rahi hai...</b>", parse_mode="HTML")
    except Exception: _pdf_status_msg = None
    ...
    try:
        ...
        if _pdf_status_msg:
            try: bot.delete_message(chat_id, _pdf_status_msg.message_id)
            except Exception: pass
...
def _countdown_and_start(chat_id, uid, quiz_id, show_countdown=True):
    ...
    send_next_poll(sid)
    if _go_msg_id:
        try: bot.delete_message(chat_id, _go_msg_id)
        except Exception: pass
...
def cmd_testseries(msg):
    """Silently redirect to /quizpdf — backward compatibility"""
    msg.text = msg.text.replace("/testseries", "/quizpdf", 1)
    cmd_quizpdf(msg)
...
def handle_document(msg):
    ...
    try: _proc_msg = bot.send_message(msg.chat.id, f"⏳ Processing `{fname}`...", parse_mode="Markdown")
    except Exception: _proc_msg = None
    try:
        raw = bot.download_file(bot.get_file(doc.file_id).file_path)
        content = raw.decode("utf-8", errors="replace")
    except Exception as exc:
        if _proc_msg:
            try: bot.delete_message(msg.chat.id, _proc_msg.message_id)
            except Exception: pass
        return safe_send(msg.chat.id, f"Download failed: {exc}")
    ...
    try: parsed, errors = detect_and_parse(fname, content)
        ...
        return safe_send(msg.chat.id, f"Parse error: {exc}")
    ...
    if not unique:
        ...
        return safe_send(msg.chat.id, f"No valid questions in `{fname}`.\n" + "\n".join(errors[:6]), parse_mode="Markdown")
    ...
    if state == "awaiting_loadfile":
        ...
        # v5.3 FIX: Double setdefault hataya — ek clean update call
...
def handle_text(msg):
    ...
    if state == "awaiting_quiz_title":
        ...
        try: _saving_msg = bot.send_message(msg.chat.id, "⏳ *Saving quiz...*", parse_mode="Markdown")
        except Exception: _saving_msg = None
        set_state(uid, "idle")
        _wizard.pop(uid, None)
        ...
        if _saving_msg:
            try: bot.delete_message(msg.chat.id, _saving_msg.message_id)
            except Exception: pass
        ...
        if not text.isdigit(): return safe_send(msg.chat.id, "Enter a valid numeric Quiz ID.")
        set_state(uid, "idle")
        threading.Thread(target=_export_pdf_quizpdf, args=(msg.chat.id, int(text)), daemon=True).start()
...
if __name__ == "__main__":
    ...
    if RENDER_URL:
        ...
        # Pehle webhook hata do (agar pehle se set ho)
        try:
            bot.remove_webhook()
            time.sleep(1)
        except Exception:
        ...
        # Flask health endpoint bhi chalao (optional for local)
